...
function renderYearly(records) {
  ...
  el.innerHTML = records.map((r, i) => `
    <div class="yearly-row reveal reveal-d${Math.min(i+1,5)}">
      <div class="yearly-row__header" role="button" tabindex="0" aria-expanded="false">
        ...
        <div class="yearly-row__summary">
          <div class="yearly-row__stat">
            <span class="yearly-row__stat-val">${r.competitions}</span>
            <span class="yearly-row__stat-label">Competitions</span>
          </div>
          <div class="yearly-row__stat">
            <span class="yearly-row__stat-val">${r.fighters_sent}</span>
            <span class="yearly-row__stat-label">Fighters Sent</span>
          </div>
        </div>
        <svg class="yearly-row__toggle" width="20" height="20" viewBox="0 0 24 24"
